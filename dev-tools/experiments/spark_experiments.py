...
"""
import logging.config, math, os, pickle, random, shutil, time, uuid
...
from slama.automl.presets.tabular_presets import SparkTabularAutoML
from slama.dataset.base import SparkDataset
from slama.ml_algo.base import SparkTabularMLAlgo
from slama.ml_algo.boost_lgbm import SparkBoostLGBM
from slama.ml_algo.linear_pyspark import SparkLinearLBFGS
from slama.pipelines.features.lgb_pipeline import SparkLGBAdvancedPipeline
from slama.pipelines.features.linear_pipeline import SparkLinearFeatures
from slama.reader.base import SparkToSparkReader
from slama.tasks.base import SparkTask as SparkTask
from slama.transformers.categorical import (
    SparkCatIntersectionsEstimator,
    SparkLabelEncoderEstimator,
    SparkTargetEncoderEstimator,
)
from slama.utils import (
    VERBOSE_LOGGING_FORMAT,
    SparkDataFrame,
    log_exec_timer,
    logging_config,
)
from slama.validation.iterators import SparkDummyIterator, SparkFoldsIterator
...
def prepare_test_and_train(
    spark: SparkSession, path: str, seed: int, test_proportion: float = 0.2
    ...
    data = data.select(
        "*", F.monotonically_increasing_id().alias(SparkDataset.ID_COLUMN), F.rand(seed).alias("is_test")
    ).cache()
    ...
    **_,
    ...
    if dataset_increase_factor > 1:
        test_data = test_data.withColumn(
            "new_col", F.explode(F.array(*[F.lit(0) for i in range(dataset_increase_factor)]))
        )
    ...
    test_metric_value = score(
        te_pred.select(
            SparkDataset.ID_COLUMN, F.col(roles["target"]).alias("target"), F.col(pred_column).alias("prediction")
        )
    )
    ...
    with log_exec_timer("spark-lama training") as train_timer:
        ...
        automl = SparkTabularAutoML(
            ...
            lgb_params={
                ...
                "default_params": {"numIterations": lgb_num_iterations, "earlyStoppingRound": 5000},
    ...
    with log_exec_timer("spark-lama ml_pipe") as pipe_timer:
        ...
        with log_exec_timer("Linear_time") as linear_timer:
            spark_ml_algo = SparkLinearLBFGS(
                cacher_key="main_cache", default_params={"regParam": [1e-5]}, freeze_defaults=True
            )
            spark_ml_algo, oof_preds = tune_and_fit_predict(spark_ml_algo, DefaultTuner(), iterator)
...
def calculate_reader(
    spark: SparkSession, path: str, task_type: str, seed: int = 42, cv: int = 5, roles: Optional[Dict] = None, **_
):
...
def calculate_le(
    ...
    with log_exec_timer("SparkLabelEncoder") as le_timer:
        estimator = SparkLabelEncoderEstimator(input_cols=list(cat_roles.keys()), input_roles=cat_roles)
...
def calculate_te(
    ...
    **_,
    ...
    if not chkp:
        ...
        data = data.withColumn("new_col", F.explode(F.array(*[F.lit(0) for i in range(dataset_increase_factor)])))
        ...
        df = df.select(
            SparkDataset.ID_COLUMN, sdataset.folds_column, sdataset.target_column, *estimator.getOutputCols()
        )
    ...
    if not chkp:
        ...
        res = {"te_fit_time": te_timer.duration, "te_transform_time": te_transform_timer.duration}
    ...
    if not chkp:
        ...
        with log_exec_timer("SparkLabelEncoder") as ci_timer:
            estimator = SparkCatIntersectionsEstimator(
                input_cols=list(cat_roles.keys()), input_roles=cat_roles, max_depth=2
            )
...
def calculate_broadcast(spark: SparkSession, **_):
    ...
    data = [{"a": i, "b": i * 10, "c": i * 100} for i in range(100)]
...
def calculate_le_scaling(spark: SparkSession, path: str, **_):
    ...
    cat_roles = {c: CategoryRole(dtype=np.float32) for c in df.columns}
...
def calculate_le_te_scaling(spark: SparkSession, path: str, checkpoint_path: Optional[str] = None, **_):
    execs = int(spark.conf.get("spark.executor.instances"))
    cores = int(spark.conf.get("spark.executor.cores"))
    ...
    if not chkp:
        ...
        le_ds = SparkDataset(
            data=df, roles=estimator.getOutputRoles(), task=SparkTask("reg"), target="target", folds="folds"
        )
    ...
    return {**result_le, "te_fit": te_timer.duration, "te_transform": te_transform_timer.duration}


def calculate_le_model_scaling(
    spark: SparkSession, path: str, model_type: str, checkpoint_path: Optional[str] = None, **_
):
    execs = int(spark.conf.get("spark.executor.instances"))
    cores = int(spark.conf.get("spark.executor.cores"))
    ...
    assembler = VectorAssembler(inputCols=le_ds.features, outputCol="assembler_features")
...
def calculate_chkp(spark: SparkSession, path: str, **_):
    ...
    return {chkp_timer.name: chkp_timer.duration}
