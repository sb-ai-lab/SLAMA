...
//        }
        // Point
        Point.Builder pointBuilder = Point.measurement(profilerName)
                .time(System.currentTimeMillis(), TimeUnit.MILLISECONDS)
                .fields(formattedMetrics);

        // Use override metaId if available, otherwise fall back to metrics metaId
        String id = metaId != null ? metaId : (String)metrics.get("metaId");
        // uncomment below line for debugging
        //logger.info("Using metaId: " + id + " (override: " + metaId + ", metrics: " + metrics.get("metaId") + ")");
        
        if (id != null) {
            pointBuilder.tag("metaId", id);
        }

        String executorId = (String)metrics.get("executorId");
        if (executorId != null) {
            pointBuilder.tag("executorId", executorId);
        }
...
//        logger.info("ping().getVersion()");
//        logger.info(this.influxDB.ping().getVersion());
//
//        if (this.influxDB.isBatchEnabled()) {
//            logger.info("isBatchEnabled true");
////            logger.info(this.influxDB.isBatchEnabled());
//        }
...
//            this.influxDB.enableBatch(1000, 1000, TimeUnit.NANOSECONDS);
            // set log level
    ...
    public void updateArguments(Map<String, List<String>> connectionProperties) {
        logger.info("Updating arguments with properties: " + connectionProperties);
        for (Map.Entry<String,  List<String>> entry : connectionProperties.entrySet()) {
            ...
            if (StringUtils.isNotEmpty(key) && value != null && !value.isEmpty()) {
                ...
                } else if (key.equals("metaId.override")) {
                    logger.info("Got value for metaId.override = "+stringValue);
                    this.metaId = stringValue;
                }
            }
        }
        logger.info("Final configuration - host: " + host + ", port: " + port + ", database: " + database + ", metaId: " + metaId);
    }
}
