...
from slama.automl.presets.base import SparkAutoMLPreset
from slama.dataset.base import SparkDataset
from slama.dataset.roles import NumericVectorOrArrayRole
from slama.ml_algo.base import AveragingTransformer, SparkMLModel, SparkTabularMLAlgo
from slama.pipelines.ml.base import SparkMLPipeline
from slama.reader.base import SparkToSparkReader
from slama.tasks.base import SparkTask
from slama.utils import SparkDataFrame
from slama.validation.base import SparkBaseTrainValidIterator
...
class DummyMLAlgo(SparkTabularMLAlgo):
    ...
    def fit_predict_single_fold(
        ...
        prediction = array_to_vector(F.array(*[F.lit(10 * 10 + j) for j in range(self.n_classes)])).alias(
            fold_prediction_column
        )
...
class DummySparkMLPipeline(SparkMLPipeline):
    def __init__(self, cacher_key: str = "", name: str = "dummy_pipe"):
    ...
    def fit_predict(self, train_valid: SparkBaseTrainValidIterator) -> SparkDataset:
        ...
        self._output_roles = {
            f"predictions_{self.name}_alg_{i}": NumericVectorOrArrayRole(
                size=n_classes,
...
class DummyTabularAutoML(SparkAutoMLPreset):
    def __init__(self, n_classes: int):
        config_path = slama.__path__ + "/automl/presets/tabular_config.yml"
    ...
    def create_automl(self, **fit_args):
        ...
        first_level = [
            SparkMLPipeline(cacher_key, ml_algos=[DummyMLAlgo(self._n_classes, name=f"dummy_0_{i}")]) for i in range(3)
        ]
        second_level = [
            SparkMLPipeline(cacher_key, ml_algos=[DummyMLAlgo(self._n_classes, name=f"dummy_1_{i}")]) for i in range(1)
        ]
