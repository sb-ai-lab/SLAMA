...
from .. import DatasetForTest, compare_sparkml_by_content, compare_sparkml_by_metadata
...
DATASETS = [
    ...
    DatasetForTest(
        "tests/unit/resources/datasets/house_prices.csv",
...
def test_target_encoder(spark: SparkSession, dataset: DatasetForTest):
    ...
    folds = pd.Series(
        np.random.choice(a=[i for i in range(CV)], size=dataset.dataset.shape[0], p=[1.0 / CV for i in range(CV)])
    )
