...
from slama.pipelines.features.lgb_pipeline import (
...
from slama.validation.iterators import SparkFoldsIterator
...
def compare_feature_pipelines_by_quality(
    spark: SparkSession,
    cv: int,
    config: Dict[str, Any],
    fp_lama_clazz,
    ...
    dump_train_path = (
        os.path.join(checkpoint_dir, f"dump_{pipeline_name}_{ds_name}_{cv}_train.dump")
        if checkpoint_dir is not None
        else None
    )
    dump_test_path = (
        os.path.join(checkpoint_dir, f"dump_{pipeline_name}_{ds_name}_{cv}_test.dump")
        if checkpoint_dir is not None
        else None
    )
    ...
    if not train_res or not test_res:
        raise ValueError(
            "Dataset should be processed with feature pipeline "
    ...
    reader = PandasToPandasReader(
        task=Task(train_valid.train.task.name), cv=cv, advanced_roles=False, random_state=seed
    )
    ...
    assert (
        spark_based_test_metric > lama_test_metric
        or abs((lama_test_metric - spark_based_test_metric) / max(lama_test_metric, spark_based_test_metric))
        < max_diff_in_percents
    )
    assert (
        spark_based_test_metric > lama_test_metric
        or abs((lama_test_metric - spark_based_test_metric) / min(lama_test_metric, spark_based_test_metric))
        < max_diff_in_percents
    )
...
def compare_feature_pipelines(
    ...
    not_equal_roles = [
        ...
        if not feat.startswith("nanflg_")
        and not (type(prole) == type(slama_pipeline.output_roles[feat]) == type(slama_feats.roles[feat]))
    ]
...
def compare_mlalgos_by_quality(
    spark: SparkSession,
    cv: int,
    config: Dict[str, Any],
    fp_lama_clazz,
...
def test_linear_features(spark: SparkSession, ds_config: Dict[str, Any], cv: int):
    compare_feature_pipelines(
        spark, cv, ds_config, LinearFeatures, SparkLinearFeatures, ml_alg_kwargs, "linear_features"
    )
...
def test_lgbadv_features(spark: SparkSession, ds_config: Dict[str, Any], cv: int):
    compare_feature_pipelines(
        spark, cv, ds_config, LGBAdvancedPipeline, SparkLGBAdvancedPipeline, ml_alg_kwargs, "lgbadv_features"
    )
...
def test_lgbsimple_features(spark: SparkSession, ds_config: Dict[str, Any], cv: int):
    compare_feature_pipelines(
        spark, cv, ds_config, LGBSimpleFeatures, SparkLGBSimpleFeatures, dict(), "lgbsimple_features"
    )
...
def test_quality_linear_features(spark: SparkSession, config: Dict[str, Any], cv: int):
    compare_feature_pipelines_by_quality(
        spark, cv, config, LinearFeatures, LinearLBFGS, ml_alg_kwargs, "linear_features"
    )
...
def test_quality_lgbadv_features(spark: SparkSession, config: Dict[str, Any], cv: int):
    compare_feature_pipelines_by_quality(
        spark, cv, config, LGBAdvancedPipeline, BoostLGBM, ml_alg_kwargs, "lgbadv_features"
    )


@pytest.mark.parametrize("config,cv", [(ds, CV) for ds in get_test_datasets(**DATASETS_ARG)])
def test_quality_lgbsimple_features(spark: SparkSession, config: Dict[str, Any], cv: int):
    compare_feature_pipelines_by_quality(spark, cv, config, LGBSimpleFeatures, BoostLGBM, dict(), "lgbsimple_features")


@pytest.mark.parametrize("config,cv", [(ds, CV) for ds in get_test_datasets(**DATASETS_ARG)])
def test_quality_mlalgo_linearlgbfs(spark: SparkSession, config: Dict[str, Any], cv: int):
    compare_mlalgos_by_quality(
        spark, cv, config, LinearFeatures, LinearLBFGS, SparkLinearLBFGS, "linear_features", ml_alg_kwargs
    )
...
def test_quality_mlalgo_boostlgbm(spark: SparkSession, config: Dict[str, Any], cv: int):
    compare_mlalgos_by_quality(
        spark, cv, config, LGBAdvancedPipeline, BoostLGBM, SparkBoostLGBM, "lgbadv_features", ml_alg_kwargs
    )
...
def test_quality_mlalgo_simple_features_boostlgbm(spark: SparkSession, config: Dict[str, Any], cv: int):
    compare_mlalgos_by_quality(
        spark, cv, config, LGBSimpleFeatures, BoostLGBM, SparkBoostLGBM, "lgbsimple_features", {}
    )
