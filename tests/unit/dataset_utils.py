...
def dump_data(path: str, ds: SparkDataset, **meta_kwargs):
    ...
    cols_to_rename = [F.col(c).alias(c.replace("(", "[").replace(")", "]")) for c in ds.data.columns]
...
def load_dump_if_exist(spark: SparkSession, path: Optional[str] = None) -> Optional[Tuple[SparkDataset, Dict]]:
    ...
    cols_to_rename = [F.col(c).alias(c.replace("[", "(").replace("]", ")")) for c in df.columns]
...
def prepared_datasets(
    spark: SparkSession, cv: int, ds_configs: List[Dict[str, Any]], checkpoint_dir: Optional[str] = None
) -> List[Tuple[SparkDataset, SparkDataset]]:
    sds = []
    for config in ds_configs:
        ...
        train_dump_path = (
            os.path.join(checkpoint_dir, f"dump_{ds_name}_{cv}_train.dump") if checkpoint_dir is not None else None
        )
        test_dump_path = (
            os.path.join(checkpoint_dir, f"dump_{ds_name}_{cv}_test.dump") if checkpoint_dir is not None else None
        )
