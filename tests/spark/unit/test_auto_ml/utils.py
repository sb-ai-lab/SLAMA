...
class DummySparkMLPipeline(SparkMLPipeline):
    def __init__(self, name: str = "dummy_pipe"):
    ...
    def fit_predict(self, train_valid: SparkBaseTrainValidIterator) -> SparkDataset:
        ...
        vshs = PipelineModel(
            stages=[VectorSizeHint(inputCol=name, size=n_classes) for name in self._output_roles.keys()]
        )
...
class DummyTabularAutoML(SparkAutoMLPreset):
    def __init__(self, n_classes: int):
        ...
        super().__init__(
            SparkTask("multiclass"), config_path=config_path, computation_settings=SequentialComputationsManager()
        )
        self._n_classes = n_classes
    ...
    def create_automl(self, **fit_args):
        ...
        first_level = [SparkMLPipeline(ml_algos=[DummyMLAlgo(self._n_classes, name=f"dummy_0_{i}")]) for i in range(3)]
        second_level = [SparkMLPipeline(ml_algos=[DummyMLAlgo(self._n_classes, name=f"dummy_1_{i}")]) for i in range(2)]
