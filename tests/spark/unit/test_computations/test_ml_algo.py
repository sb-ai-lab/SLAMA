...
manager_mlalgo_configs = [
    (manager, mlalgo_builder()) for manager, mlalgo_builder in itertools.product(manager_configs, ml_algo_configs)
]


@pytest.mark.parametrize("manager,ml_algo", manager_mlalgo_configs)
def test_ml_algo(
    spark: SparkSession, dataset: SparkDataset, manager: Optional[ComputationsManager], ml_algo: SparkTabularMLAlgo
):
