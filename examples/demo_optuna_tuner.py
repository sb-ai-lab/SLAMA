...
if __name__ == "__main__":
    ...
    dataset_sdf = dataset_sdf.select("*", F.monotonically_increasing_id().alias(SparkDataset.ID_COLUMN)).cache()
    dataset_sdf.write.mode("overwrite").format("noop").save()
    dataset_sdf = dataset_sdf.select(
        F.col("__fold__").cast("int").alias("__fold__"), *[c for c in dataset_sdf.columns if c != "__fold__"]
    )
